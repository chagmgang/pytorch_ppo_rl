...
class CNNRNDActorCriticNetwork(nn.Module):
    def __init__(self):
        super(CNNRNDActorCriticNetwork, self).__init__()
        ...
        self.critic_ex = nn.Linear(256, 1)
        self.critic_in = nn.Linear(256, 1)
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc2(x))
        policy = self.actor(x)
        value_ex = self.critic_ex(x)
        value_in = self.critic_in(x)
        return policy, value_ex, value_in
